...
const ORGS = {
  once: {
    name: 'Campaign recharge once',
    email: 'campaign-ar-once@drill.test',
    card: CARDS.visa,
    last4: '4242'
  },
  cap: {
    name: 'Campaign recharge cap',
    email: 'campaign-ar-cap@drill.test',
    card: CARDS.visa,
    last4: '4242'
  },
  decline: {
    name: 'Campaign recharge decline',
    email: 'campaign-ar-decline@drill.test',
    card: CARDS.chargeFails,
    last4: '0341'
  }
};
...
function need(key) {
  ...
  if (!m?.org || !m?.customerId) {
    throw new CampaignError(
      `the organization "${ORGS[key].name}" is missing: the first scenario (the card setups) did not complete`,
      { key }
    );
  }
  return m;
...
async function postAccepted(ctx, events, what) {
  ...
  if (bad.length || out.accepted !== events.length) {
    throw new CampaignError(`${what}: not every event was accepted`, {
      results: out.results,
...
async function debit(ctx, org, credits) {
  ...
  if (r.credits !== credits)
    throw new CampaignError(`the debit of ${credits} was priced ${r.credits}`, { result: r });
  return r;
}
...
export const scenarios = [
  {
    ...
    async run(ctx) {
      ...
      for (const [key, spec] of Object.entries(ORGS)) {
        ...
        if (!accountId)
          throw new CampaignError(`the organization "${spec.name}" has no billing account`, { org });
        ...
        if (!setup.url || !setup.sessionId)
          throw new CampaignError(`the setup session of "${spec.name}" carries no url`, {
            org,
            sessionId: setup.sessionId ?? null
          });
        ctx.log(
          `auto-recharge: ${spec.name} (${org}) saves a card ending ${spec.last4} through setup session ${setup.sessionId}`
        );
        ...
        if (!account) {
          throw new CampaignError(
            `the account of "${spec.name}" never showed the card ending ${spec.last4}`,
            {
              org,
        ...
        if (!customerId)
          throw new CampaignError(`the hub holds no Stripe customer for "${spec.name}" after the setup`, {
            org,
            sessionId: setup.sessionId
          });
        ...
        if (pm?.card?.last4 !== spec.last4) {
          throw new CampaignError(
            `the Stripe customer of "${spec.name}" has no default card ending ${spec.last4}`,
            { customerId, defaultLast4: pm?.card?.last4 ?? null }
          );
        }
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const t0 = Date.now();
      const set = await setAutoRecharge(
        ctx,
        org,
        { enabled: true, threshold: 4000, credits: 20000, monthlyCap: 60000 },
        'turning auto-recharge on'
      );
      const ar = set.autoRecharge;
      if (ar?.enabled !== true || ar.threshold !== 4000 || ar.credits !== 20000 || ar.monthlyCap !== 60000) {
        throw new CampaignError('the account read does not echo the auto-recharge settings', {
          autoRecharge: ar
        });
      }
      ...
      if (start - 1500 >= 4000)
        throw new CampaignError(
          'the starting balance is too high for a debit of 1,500 to cross the threshold',
          { start }
        );
      const debited = await debit(ctx, org, 1500);
      const purchase = await ctx.hub.waitLedger(org, { kind: 'purchase', amount: 20000 });
      if (!purchase)
        throw new CampaignError('no purchase of 20,000 landed within 90 s of the debit', {
          debitId: debited.id,
          runs: await ctx.hub.runs(org),
      ...
      if (!runs || runs.length !== 1 || runs[0].state !== 'succeeded') {
        throw new CampaignError('the account does not hold exactly one succeeded run', {
          runs: runs ?? (await ctx.hub.runs(org))
        });
      }
      const run = runs[0];
      if (run.paymentIntentId !== purchase.sourceRef) {
        throw new CampaignError('the run and the purchase name different payment intents', {
          runPaymentIntentId: run.paymentIntentId,
          purchaseSourceRef: purchase.sourceRef
        });
      }
      ...
      const intentFacts = {
        ...
        credits: intent.metadata?.credits,
        runId: intent.metadata?.runId ?? null,
      ...
      if (
        ...
        intent.amount !== 2000 ||
        intent.currency !== 'eur' ||
        intent.metadata?.kind !== 'auto_recharge' ||
        intent.metadata?.credits !== '20000'
      ) {
        throw new CampaignError(
          'the Stripe intent of the run is not a succeeded EUR 20.00 auto-recharge of 20,000 credits',
          { intent: intentFacts }
        );
      }
      if (intent.customer !== customerId)
        throw new CampaignError('the intent was charged to another customer', {
          intent: intentFacts,
          customerId
        });
      const purchases = await ctx.hub.ledger(org, 'purchase');
      if (purchases.length !== 1)
        throw new CampaignError('the ledger holds more than one purchase', { purchases });
      const expected = start - 1500 + 20000;
      const account = await ctx.hub.account(org);
      if (account.balance !== expected)
        throw new CampaignError(`the balance reads ${account.balance}, expected ${expected}`, {
          start,
          balance: account.balance
        });
      if (expected !== 23500)
        ctx.report.note(
          `auto-recharge: the once organization started at ${start}, not 5,000; the balance was judged against ${expected}.`
        );
      if (account.autoRecharge?.rechargedThisMonth !== 20000) {
        throw new CampaignError('rechargedThisMonth does not read 20,000', {
      ...
      if (lowMails.length)
        throw new CampaignError('a low-balance mail went out although auto-recharge answered', {
          mails: lowMails.map((m) => ({ id: m.ID, subject: m.Subject }))
        });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (runsBefore.length !== 1 || purchasesBefore.length !== 1) {
        throw new CampaignError('the previous scenario did not leave exactly one run and one purchase', {
          runs: runsBefore,
          purchases: purchasesBefore.length
        });
      }
      const set = await setAutoRecharge(ctx, org, { threshold: 30000 }, 'raising the threshold to 30,000');
      if (set.autoRecharge?.threshold !== 30000)
        throw new CampaignError('the account read does not echo the threshold 30,000', {
          autoRecharge: set.autoRecharge
        });
      const start = set.balance;
      if (start >= 30000)
        throw new CampaignError('the balance is not under the raised threshold', { balance: start });
      ...
      if (!purchases)
        throw new CampaignError('no second purchase landed after the two debits', {
          runs: await ctx.hub.runs(org),
      ...
      if (runs.length !== 2 || succeeded.length !== 2)
        throw new CampaignError('the account does not hold exactly two runs, both succeeded', { runs });
      if (purchases.length !== 2)
        throw new CampaignError('the ledger holds more than two purchases', {
          purchases: purchases.map((p) => ({ id: p.id, amount: p.amount, sourceRef: p.sourceRef }))
        });
      const intents = await succeededRechargeIntents(ctx, customerId);
      if (intents.length !== 2) {
        throw new CampaignError(
          'Stripe does not hold exactly two succeeded auto-recharge intents for the customer',
          {
            customerId,
            intents: intents.map((pi) => ({ id: pi.id, amount: pi.amount, runId: pi.metadata?.runId }))
          }
        );
      }
      ...
      if (newRun.paymentIntentId !== newPurchase.sourceRef) {
        throw new CampaignError('the new run and the new purchase name different payment intents', {
          run: newRun,
          purchaseSourceRef: newPurchase.sourceRef
        });
      }
      ...
      if (expected !== 43490)
        ctx.report.note(
          `auto-recharge: the concurrent-debit scenario started at ${start}, not 23,500; the balance was judged against ${expected}.`
        );
      const agrees = await ctx.hub.balanceAgrees(org);
      if (!agrees.agrees) throw new CampaignError('the balance is not the sum of the ledger', { agrees });
      return {
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const set = await setAutoRecharge(
        ctx,
        org,
        { enabled: true, threshold: 4900, credits: 20000, monthlyCap: 20000 },
        'turning auto-recharge on with a cap of one recharge'
      );
      const ar = set.autoRecharge;
      ...
      if (start - 500 >= 4900)
        throw new CampaignError(
          'the starting balance is too high for a debit of 500 to cross the threshold',
          { start }
        );
      await debit(ctx, org, 500);
      const first = await ctx.hub.waitLedger(org, { kind: 'purchase', amount: 20000 });
      if (!first)
        throw new CampaignError('the first recharge did not land within 90 s', {
          runs: await ctx.hub.runs(org)
        });
      const afterFirst = await waitAccount(ctx, org, (a) => a.balance === start - 500 + 20000, 30_000);
      if (!afterFirst)
        throw new CampaignError(`the balance does not read ${start - 500 + 20000} after the first recharge`, {
          balance: (await ctx.hub.account(org)).balance
        });

      const raised = await setAutoRecharge(ctx, org, { threshold: 30000 }, 'raising the threshold to 30,000');
      if (raised.autoRecharge?.threshold !== 30000)
        throw new CampaignError('the account read does not echo the threshold 30,000', {
          autoRecharge: raised.autoRecharge
        });
      await debit(ctx, org, 5);
      const paused = await waitAccount(ctx, org, (a) => a.autoRecharge?.disabledReason === 'monthly_cap');
      if (!paused)
        throw new CampaignError('the account never read disabledReason monthly_cap', {
          autoRecharge: (await ctx.hub.account(org)).autoRecharge,
          runs: await ctx.hub.runs(org)
        });
      if (paused.autoRecharge.enabled !== true)
        throw new CampaignError('the cap switched auto-recharge off instead of pausing it', {
          autoRecharge: paused.autoRecharge
        });
      await sleep(15_000);
      ...
      if (purchasesPaused.length !== 1 || succeededPaused.length !== 1) {
        throw new CampaignError('the cap did not hold: a second recharge happened', {
          purchases: purchasesPaused.length,
          runs: runsPaused
        });
      }
      ...
      if (!capMail)
        throw new CampaignError('no cap mail reached the billing email', {
          to: ORGS.cap.email,
          subject: capSubject
        });

      const reopened = await setAutoRecharge(ctx, org, { monthlyCap: 60000 }, 'raising the cap to 60,000');
      if (reopened.autoRecharge?.monthlyCap !== 60000)
        throw new CampaignError('the account read does not echo the cap 60,000', {
          autoRecharge: reopened.autoRecharge
        });
      await debit(ctx, org, 5);
      const purchases = await waitPurchases(ctx, org, 2);
      if (!purchases)
        throw new CampaignError('no second recharge landed after the cap was raised', {
          runs: await ctx.hub.runs(org),
          autoRecharge: (await ctx.hub.account(org)).autoRecharge
        });
      ...
      if (!cleared)
        throw new CampaignError('disabledReason did not clear after the admitted recharge', {
          autoRecharge: (await ctx.hub.account(org)).autoRecharge
        });
      const runs = await ctx.hub.runs(org);
      const succeeded = runs.filter((r) => r.state === 'succeeded');
      if (purchases.length !== 2 || succeeded.length !== 2)
        throw new CampaignError('the account does not hold exactly two purchases and two succeeded runs', {
          purchases: purchases.length,
          runs
        });
      const expected = start - 500 - 5 - 5 + 20000 + 20000;
      if (cleared.balance !== expected)
        throw new CampaignError(`the balance reads ${cleared.balance}, expected ${expected}`, {
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const set = await setAutoRecharge(
        ctx,
        org,
        { enabled: true, threshold: 4000, credits: 20000, monthlyCap: 40000 },
        'turning auto-recharge on'
      );
      if (set.autoRecharge?.enabled !== true || set.autoRecharge.threshold !== 4000)
        throw new CampaignError('the account read does not echo the auto-recharge settings', {
          autoRecharge: set.autoRecharge
        });
      if (set.balance - 1500 >= 4000)
        throw new CampaignError(
          'the starting balance is too high for a debit of 1,500 to cross the threshold',
          { start: set.balance }
        );
      await debit(ctx, org, 1500);
      const parked = await waitAccount(
        ctx,
        org,
        (a) => a.autoRecharge?.disabledReason === 'declined_retrying'
      );
      if (!parked)
        throw new CampaignError('the account never read disabledReason declined_retrying', {
          autoRecharge: (await ctx.hub.account(org)).autoRecharge,
          runs: await ctx.hub.runs(org)
        });
      if (parked.autoRecharge.enabled !== true)
        throw new CampaignError('the first decline switched auto-recharge off instead of parking a retry', {
          autoRecharge: parked.autoRecharge
        });
      const runs = await ctx.hub.runs(org);
      if (runs.length !== 1 || runs[0].state !== 'retry')
        throw new CampaignError('the account does not hold exactly one run in state retry', { runs });
      const run = runs[0];
      if (!run.reason) throw new CampaignError('the parked run carries no reason', { run });
      const retryAt = run.retryAtMs;
      const hours = (retryAt - Date.now()) / 3_600_000;
      if (!Number.isFinite(hours) || hours < 23 || hours > 25)
        throw new CampaignError('the retry is not set between 23 and 25 hours from now', {
          retryAt: run.retryAt,
          hours
        });
      const mail = await ctx.mail.waitFor(
        { to: ORGS.decline.email, subject: 'Auto-recharge', after: declineT0 },
        60_000
      );
      if (!mail)
        throw new CampaignError('no auto-recharge mail reached the billing email', {
          to: ORGS.decline.email
        });
      ...
      if (!/one more try|try once more|retry|try again/i.test(`${mail.Subject}\n${mail.Text ?? ''}`))
        throw new CampaignError('the auto-recharge mail does not name a retry', {
          mailId: mail.ID,
          subject: mail.Subject,
          text: (mail.Text ?? '').slice(0, 400)
        });
      const purchases = await ctx.hub.ledger(org, 'purchase');
      if (purchases.length)
        throw new CampaignError('the ledger holds a purchase although the card was declined', { purchases });
      let intentStatus = null;
      if (run.paymentIntentId) {
        const intent = await ctx.stripe.paymentIntent(run.paymentIntentId);
        intentStatus = intent.status;
        if (intent.status === 'succeeded')
          throw new CampaignError('the Stripe intent of the declined run succeeded', {
            paymentIntentId: intent.id
          });
      } else {
        ctx.report.note(
          'auto-recharge: the declined run records no payment intent id; the check fell back to the customer having no succeeded auto-recharge intent.'
        );
      }
      const succeeded = await succeededRechargeIntents(ctx, customerId);
      if (succeeded.length)
        throw new CampaignError('Stripe holds a succeeded auto-recharge intent for the declined customer', {
          intents: succeeded.map((pi) => pi.id)
        });
      declineRun = run;
      return {
        org,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (!declineRun)
        throw new CampaignError('the parked run is missing: the declined-card scenario did not complete', {});
      const moved = await ctx.hub.retryNow(org);
      if (!moved.split('\n').includes(declineRun.id))
        throw new CampaignError('the parked run could not be brought forward', {
          runId: declineRun.id,
          moved
        });
      const jobId = await ctx.hub.runSweep();
      const off = await waitAccount(
        ctx,
        org,
        (a) => a.autoRecharge?.enabled === false && a.autoRecharge?.disabledReason === 'declined'
      );
      if (!off)
        throw new CampaignError('auto-recharge did not switch off with the reason declined', {
          jobId,
          autoRecharge: (await ctx.hub.account(org)).autoRecharge,
          runs: await ctx.hub.runs(org)
        });
      ...
      if (theRun?.state !== 'failed')
        throw new CampaignError('the parked run is not failed after the retry', { runs });
      const live = runs.filter((r) => r.state !== 'failed');
      if (live.length)
        throw new CampaignError('a run other than a failed one is left on the account', { runs });
      const offSubject = `Auto-recharge is off for ${ORGS.decline.name}`;
      const offMail = await ctx.mail.waitFor(
        { to: ORGS.decline.email, subject: offSubject, after: declineT0 },
        60_000
      );
      if (!offMail)
        throw new CampaignError('no switched-off mail reached the billing email', {
          to: ORGS.decline.email,
          subject: offSubject
        });
      const allMails = await mailsOf(ctx, ORGS.decline.email, 'Auto-recharge', declineT0);
      if (allMails.length !== 2)
        throw new CampaignError('the billing email did not receive exactly two auto-recharge mails', {
          mails: allMails.map((m) => ({ id: m.ID, subject: m.Subject }))
        });
      if ((await ctx.hub.ledger(org, 'purchase')).length)
        throw new CampaignError('the ledger holds a purchase after the second decline', {});
      ...
      if (runsAfter.length !== runsBefore || purchasesAfter.length) {
        throw new CampaignError('a debit after the switch-off started a recharge', {
          runsBefore,
          runsAfter,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const removed = expectStatus(
        await ctx.hub.paymentMethodRemove(org),
        200,
        'DELETE /billing/payment-method'
      ).json;
      if (removed.paymentMethod !== null)
        throw new CampaignError('the account read still shows a card', {
          paymentMethod: removed.paymentMethod
        });
      if (
        removed.autoRecharge?.enabled !== false ||
        removed.autoRecharge?.disabledReason !== 'no_payment_method'
      ) {
        throw new CampaignError('auto-recharge is not off with no_payment_method', {
          autoRecharge: removed.autoRecharge
        });
      }
      const again = await ctx.hub.autoRecharge(org, { enabled: true });
      if (again.status !== 409 || codeOf(again) !== 'payment_method_missing') {
        throw new CampaignError(
          'turning auto-recharge on without a card was not refused 409 payment_method_missing',
          { status: again.status, body: again.json ?? again.text.slice(0, 400) }
        );
      }
      const pm = await ctx.stripe.defaultPaymentMethod(customerId);
      if (pm !== null)
        throw new CampaignError('the Stripe customer still has a default payment method', {
          customerId,
      ...
      return {
        ...
        customerId,
        autoRecharge: removed.autoRecharge,
        refusal: { status: again.status, code: codeOf(again) },
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (
        capRes.status !== 400 ||
        codeOf(capRes) !== 'validation_error' ||
        !capRes.text.includes('monthlyCap')
      ) {
        throw new CampaignError(
          'a cap under one recharge was not refused 400 validation_error naming monthlyCap',
          { status: capRes.status, body: capRes.json ?? capRes.text.slice(0, 400) }
        );
      }
      ...
      if (typeof floor !== 'number' || floor <= 1) {
        ctx.report.note(
          `auto-recharge: the recharge floor reads ${floor ?? 'nothing'} (customMinCredits), so a recharge of 1 credit is not below it; the floor refusal was not judged.`
        );
        evidence.floorJudged = false;
        return evidence;
      }
      if (
        creditsRes.status !== 400 ||
        codeOf(creditsRes) !== 'validation_error' ||
        !creditsRes.text.includes('credits')
      ) {
        throw new CampaignError(
          `a recharge of 1 credit (under the floor ${floor}) was not refused 400 validation_error naming credits`,
          { status: creditsRes.status, body: creditsRes.json ?? creditsRes.text.slice(0, 400) }
        );
      }
      ...
      evidence.storedAutoRecharge = {
        credits: after.autoRecharge.credits,
        monthlyCap: after.autoRecharge.monthlyCap
      };
      return evidence;
    }
  }
