...
function state(ctx, ...needed) {
  ...
  if (missing.length) throw new CampaignError(`the plan group state lacks ${missing.join(', ')}: an earlier scenario did not complete`, { missing });
  return s;
}
...
async function waitAccount(ctx, org, test, what, timeoutMs = 90_000) {
  ...
  if (!hit) throw new CampaignError(`${what}: the account read never matched in ${Math.round(timeoutMs / 1000)} s`, { org, last: last ? planView(last) : null });
  return hit;
}
...
function proPriceOf(sub) {
  ...
  return { priceId: price?.id ?? null, lookupKey: price?.lookup_key ?? null, unitAmount: price?.unit_amount ?? null, currency: price?.currency ?? null };
}
...
async function sweepToFree(ctx, org, accountId, paidEnd, what) {
  ...
  if (account.plan !== 'pro' || !paidEndAhead || !sameInstant(account.planUntil, paidEnd)) {
    throw new CampaignError(`${what}: after the sweep the account is neither free nor Pro to the paid end`, { org, paidEnd, account: beforeShift, hubNow: new Date().toISOString() });
  }
  const moved = await bringPaidEndToNow(ctx, accountId);
  if (!moved) throw new CampaignError(`${what}: the paid end could not be moved on the hub row`, { accountId });
  ctx.report.note(
...
export const scenarios = [
  {
    ...
    async run(ctx) {
      ...
      if (!ws.workspaceId || !ws.org) throw new CampaignError('the Slideless workspace does not project a hub organization', { ws });
      ...
      if (start.plan !== 'free') throw new CampaignError('the new organization is not on the free plan', { account: planView(start) });
      if (start.balance !== 5000) ctx.report.note(`plan: the new organization started at ${start.balance} credits, not 5,000; the balance check uses the start read.`);
      ...
      if (live.length !== 10) throw new CampaignError('the deck does not list exactly 10 live share links', { live: live.length, all: links.length });
      ...
      if (!settled) throw new CampaignError(`the balance never read ${expected} (the start minus 50 for the deck, 5 for its page, 200 for ten links)`, { start: start.balance, expected, last: last?.balance, ledger: (await ctx.hub.ledger(ws.org)).slice(0, 15) });
      return {
...

  {
    ...
    async run(ctx) {
      ...
      if (!Slideless.isPlanRefusal(eleventh, 'links.perDeck')) throw new CampaignError('the eleventh link was not refused plan_required on links.perDeck', { status: eleventh.status, body: eleventh.json ?? eleventh.text.slice(0, 400) });
      const locked = await ctx.sl.mintLink(workspaceId, s.deckId, { name: 'locked', password: LOCKED_PASSWORD });
      if (!Slideless.isPlanRefusal(locked, 'deck.password')) throw new CampaignError('the password link was not refused plan_required on deck.password', { status: locked.status, body: locked.json ?? locked.text.slice(0, 400) });
      ...
      if (m4.status !== 403 || m4.json?.error?.code !== 'plan_required' || d.key !== 'workspace.members' || typeof d.upgradeUrl !== 'string' || !d.upgradeUrl.includes('tool=slideless-cloud')) {
        throw new CampaignError('the fourth seat was not refused plan_required on workspace.members with an upgrade link naming slideless-cloud', { status: m4.status, body: m4.json ?? m4.text.slice(0, 400) });
      }

      await ctx.sl.waitUsageDrained();
      const eventsAfter = await usageEventCount(ctx, s.accountId);
      if (eventsAfter !== eventsBefore) throw new CampaignError('the refusals queued usage events at the hub', { eventsBefore, eventsAfter });
      return {
        eleventh: { status: eleventh.status, key: eleventh.json.error.details.key, requiredPlan: eleventh.json.error.details.requiredPlan },
        password: { status: locked.status, key: locked.json.error.details.key, requiredPlan: locked.json.error.details.requiredPlan },
        invites: { m2: m2.status, m3: m3.status, m4: m4.status },
        seatRefusal: { key: d.key, plan: d.plan, requiredPlan: d.requiredPlan ?? null, upgradeUrl: d.upgradeUrl },
        eventsBefore,
        eventsAfter
      };
    }
...

  {
    ...
    async run(ctx) {
      ...
      const paid = await ctx.payCheckout(res.json.url, { mode: 'subscription', card: CARDS.visa, email: EMAIL_A, label: 'plan-subscribe' });
      const account = await waitAccount(
        ctx,
        org,
        (a) => a.plan === 'pro' && a.subscription?.status === 'active' && a.subscription?.cancelAtPeriodEnd === false && a.planUntil === null,
        'the plan did not flip to a live Pro'
      );
      const heldId = await ctx.hub.sql(`SELECT coalesce(stripe_subscription_id, '') FROM billing_accounts WHERE id = '${s.accountId}'`);
      const subs = (await ctx.stripe.subscriptions(customerId)).data ?? [];
      const sub = subs.find((x) => x.id === heldId) ?? null;
      if (!sub) throw new CampaignError('the subscription the hub holds is not among the customer\'s subscriptions', { heldId, stripe: subs.map((x) => ({ id: x.id, status: x.status })) });
      if (subs.length !== 1) throw new CampaignError('the customer carries more than one subscription', { stripe: subs.map((x) => ({ id: x.id, status: x.status })) });
      const price = proPriceOf(sub);
      if (price.lookupKey !== 'antasphere-pro-monthly-eur' || price.unitAmount !== 5000) throw new CampaignError('the subscription is not on the EUR Pro monthly price', { price });
      ...
      if (again.json?.url !== null) throw new CampaignError('a second subscribe opened another Checkout beside the live subscription', { body: again.json });
      return {
        customerId,
...

  {
    ...
    async run(ctx) {
      ...
      while (Date.now() - t0 < 90_000) {
        ...
        if (!Slideless.isPlanRefusal(eleventh, 'links.perDeck')) throw new CampaignError('the eleventh link answered something other than 201 or the plan refusal', { status: eleventh.status, body: eleventh.json ?? eleventh.text.slice(0, 400) });
        refusals += 1;
        await sleep(3000);
      }
      if (eleventh?.status !== 201) throw new CampaignError('the eleventh link was still refused 90 s after the plan flipped to Pro', { status: eleventh?.status, body: eleventh?.json, refusals });
      const unlockedAfterMs = Date.now() - t0;

      const locked = await ctx.sl.mintLink(workspaceId, s.deckId, { name: 'locked', password: LOCKED_PASSWORD });
      expectStatus(locked, 201, 'the password link on Pro');
      if (locked.json?.shareToken?.hasPassword !== true) throw new CampaignError('the password link does not say hasPassword', { shareTokenId: locked.json?.shareToken?.id, hasPassword: locked.json?.shareToken?.hasPassword });
      ...
      if (ent.plan !== 'pro' || !hasFeature(ent.features, 'deck.password')) throw new CampaignError('the hub entitlements do not read Pro with deck.password', { plan: ent.plan, features: ent.features });
      return {
...

  {
    ...
    async run(ctx) {
      ...
      const paidEvent = await ctx.relay.waitProcessed((r) => r.type === 'invoice.paid' && r.body.includes(s.subscriptionId) && r.receivedAt > t0);
      if (!paidEvent) {
        throw new CampaignError('no processed invoice.paid for the renewal reached the relay in 90 s', {
          subscriptionId: s.subscriptionId,
          seen: ctx.relay.records.filter((r) => r.receivedAt > t0 && r.body.includes(s.subscriptionId)).map((r) => ({ type: r.type, outcome: r.outcome, status: r.status }))
        });
      }
      const account = await waitAccount(
        ctx,
      ...
      if (account.plan !== 'pro' || account.planUntil !== null) throw new CampaignError('the renewed account is not a live Pro', { account: planView(account) });
      const sub = await ctx.stripe.subscription(s.subscriptionId);
      if (sub.status !== 'active') throw new CampaignError('the Stripe subscription is not active after the renewal', { status: sub.status });
      ...
      if (latest?.status !== 'paid') throw new CampaignError('the subscription\'s latest invoice is not paid', { latestId, status: latest?.status });
...

  {
    ...
    async run(ctx) {
      ...
      if (cancel.json?.subscription?.cancelAtPeriodEnd !== true) throw new CampaignError('cancel did not answer a pending cancellation', { body: cancel.json });
      const cancelled = await ctx.hub.account(org);
      if (cancelled.plan !== 'pro' || !sameInstant(cancelled.planUntil, cancelled.subscription?.currentPeriodEnd)) {
        throw new CampaignError('after cancel the account is not Pro until the paid period end', { account: planView(cancelled) });
      }
      const stripeCancelled = await ctx.stripe.subscription(s.subscriptionId);
      if (stripeCancelled.cancel_at_period_end !== true) throw new CampaignError('Stripe does not carry the pending cancellation', { cancelAtPeriodEnd: stripeCancelled.cancel_at_period_end });

      const resume = await ctx.hub.subscription(org, 'resume');
      expectStatus(resume, 200, 'resume');
      if (resume.json?.subscription?.cancelAtPeriodEnd !== false) throw new CampaignError('resume did not clear the pending cancellation', { body: resume.json });
      const resumed = await ctx.hub.account(org);
      if (resumed.plan !== 'pro' || resumed.planUntil !== null) throw new CampaignError('after resume the account is not a live Pro', { account: planView(resumed) });
      const stripeResumed = await ctx.stripe.subscription(s.subscriptionId);
      if (stripeResumed.cancel_at_period_end !== false) throw new CampaignError('Stripe still carries the cancellation after resume', { cancelAtPeriodEnd: stripeResumed.cancel_at_period_end });
      return {
        subscriptionId: s.subscriptionId,
        afterCancel: planView(cancelled),
        afterResume: planView(resumed),
        stripe: { afterCancel: stripeCancelled.cancel_at_period_end, afterResume: stripeResumed.cancel_at_period_end }
      };
    }
  },

  {
    ...
    async run(ctx) {
      ...
      if (!org || !accountId) throw new CampaignError('the renewal organization is missing: scenario 1 did not complete', { renewal: s.renewal });
      ...
      if (!res.json?.url) throw new CampaignError('subscribe answered no Checkout url (renewal org)', { body: res.json });
      const customerId = await ctx.hub.customerOf(org);
      if (!customerId) throw new CampaignError('the hub made no Stripe customer on subscribe (renewal org)', { org });
      await ctx.stripe.tagCustomer(customerId);
      await ctx.payCheckout(res.json.url, { mode: 'subscription', card: CARDS.visa, email: EMAIL_B, label: 'plan-renewal-subscribe' });
      ...
      const subId = await ctx.hub.sql(`SELECT coalesce(stripe_subscription_id, '') FROM billing_accounts WHERE id = '${accountId}'`);
      if (!subId) throw new CampaignError('the hub holds no subscription id for the renewal organization', { accountId });
      ...
      const failed = await ctx.relay.waitProcessed((r) => r.type === 'invoice.payment_failed' && r.body.includes(subId) && r.receivedAt > t1);
      if (!failed) {
        throw new CampaignError('no processed invoice.payment_failed for the renewal reached the relay in 90 s', {
          subId,
          seen: ctx.relay.records.filter((r) => r.receivedAt > t1 && r.body.includes(subId)).map((r) => ({ type: r.type, outcome: r.outcome, status: r.status }))
        });
      }
      ...
      if (!firstMail) throw new CampaignError('the failed renewal mail did not arrive', { to: EMAIL_B, subject });
      ...
      const retry = await ctx.relay.waitProcessed((r) => r.type === 'invoice.payment_failed' && r.body.includes(subId) && r.receivedAt > t2, { timeoutMs: 30_000 });
      await sleep(5000);
      const mails = await ctx.mail.find({ to: EMAIL_B, subject, after: tStart });
      if (mails.length !== 1) throw new CampaignError('the failed renewal mail did not arrive exactly once', { count: mails.length, ids: mails.map((m) => m.ID), retryEvent: retry?.id ?? null });

      await ctx.stripe.advanceClock(customerId, 40 * DAY);
      const stripeSub = await ctx.stripe.subscription(subId);
      const { account, clockShift, beforeShift } = await sweepToFree(ctx, org, accountId, paidEnd, 'a failed renewal keeps pro to the paid end, then drops to free');
      const free = account.plan === 'free' && account.planUntil === null ? account : await waitAccount(ctx, org, (a) => a.plan === 'free' && a.planUntil === null, 'the lapsed renewal did not drop to free');
      return {
        customerId,
...

  {
    ...
    async run(ctx) {
      ...
      const cancelled = await ctx.hub.account(org);
      if (cancelled.plan !== 'pro' || !cancelled.planUntil || !sameInstant(cancelled.planUntil, cancelled.subscription?.currentPeriodEnd)) {
        throw new CampaignError('after cancel the account is not Pro until the paid period end', { account: planView(cancelled) });
      }
      ...
      const deleted = await ctx.relay.waitProcessed((r) => r.type === 'customer.subscription.deleted' && r.body.includes(s.subscriptionId) && r.receivedAt > t0);
      if (!deleted) {
      ...
      if (stripeSub.status !== 'canceled') throw new CampaignError('the Stripe subscription is not canceled after its period', { status: stripeSub.status });
      const { account, clockShift, beforeShift } = await sweepToFree(ctx, org, s.accountId, paidEnd, 'downgrade: after the paid period a canceled subscription is free');
      const free = account.plan === 'free' && account.subscription === null ? account : await waitAccount(ctx, org, (a) => a.plan === 'free' && a.subscription === null, 'the canceled subscription did not leave a free account without a subscription');
      ...
      while (Date.now() - t1 < 90_000) {
        ...
        else throw new CampaignError('the twelfth link answered something other than 201 or the plan refusal', { status: twelfth.status, body: twelfth.json ?? twelfth.text.slice(0, 400) });
        await sleep(3000);
      }
      if (!Slideless.isPlanRefusal(twelfth, 'links.perDeck')) throw new CampaignError('Slideless still minted links 90 s after the downgrade', { status: twelfth?.status, mintedMeanwhile });

      const links = await ctx.sl.links(workspaceId, s.deckId);
      const locked = links.find((t) => t.id === s.lockedId);
      if (!locked || locked.hasPassword !== true || locked.revokedAt) throw new CampaignError('the password link minted on Pro is not still live with its password', { lockedId: s.lockedId, found: locked ? { hasPassword: locked.hasPassword, revokedAt: locked.revokedAt } : null });
      return {
        subscriptionId: s.subscriptionId,
        paidEnd,
