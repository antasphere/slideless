...
const ORGS = {
  ...
  frb: {
    ...
    details: {
      billingEmail: 'campaign-frb@drill.test',
      companyName: 'Campagne SAS',
...
function state(ctx) {
  ...
  if (!s || !s.orgs)
    throw new CampaignError(
      'the buying organizations are missing: the first scenario of the group did not run or failed',
      {}
    );
  return s;
}
...
function invoiceTaxLines(inv) {
  if (Array.isArray(inv.total_tax_amounts))
    return { field: 'total_tax_amounts', lines: inv.total_tax_amounts };
...
async function buy(ctx, key, body, { credits, subtotal, label }) {
  ...
  if (paid.outcome !== 'paid')
    throw new CampaignError('the Checkout page did not report paid', {
      sessionId,
  ...
  if (!invoiceId)
    throw new CampaignError('the paid session carries no invoice (invoice creation is off?)', sessionFacts);
  if (session.currency !== spec.currency.toLowerCase()) {
    throw new CampaignError(
      `the session is in ${session.currency}, expected ${spec.currency.toLowerCase()}`,
      sessionFacts
    );
  }
  ...
  if (!Number.isInteger(metaCredits) || metaCredits !== credits) {
    throw new CampaignError(
      `the session metadata carries ${session.metadata?.credits} credits, expected ${credits}`,
      sessionFacts
    );
  }
  ...
  if (typeof tax !== 'number' || Math.abs(tax - expectedTax) > 1) {
    throw new CampaignError(
      `the tax is ${tax}, expected ${expectedTax} (${spec.vatPct} % of ${subtotal}, plus or minus 1)`,
      { ...sessionFacts, breakdown: session.total_details?.breakdown ?? null }
    );
  }
  if (session.amount_total !== subtotal + tax) {
    throw new CampaignError(
      `the total ${session.amount_total} is not the subtotal ${subtotal} plus the tax ${tax}`,
      sessionFacts
    );
  }
  ...
  if (!entry)
    throw new CampaignError('no purchase entry for the payment intent reached the ledger in 90 s', {
      ...sessionFacts,
      ledger: (await ctx.hub.ledger(org)).slice(0, 10)
    });
  if (entry.amount !== metaCredits || entry.amount !== credits) {
    throw new CampaignError(
      `the purchase entry is ${entry.amount} credits, the metadata says ${metaCredits}, the request asked ${credits}`,
      { entry, ...sessionFacts }
    );
  }
  ...
  if (!rows) {
    throw new CampaignError(
      'the stripe_events rows of the payment intent are missing or not all processed after 60 s',
      {
        paymentIntentId,
        rows: await ctx.hub.eventRows(paymentIntentId),
        relay: ctx.relay
          .ofObject(paymentIntentId)
  ...
  if (purchases.length !== 1)
    throw new CampaignError(
      `${purchases.length} purchase entries carry the payment intent, expected exactly 1`,
      { paymentIntentId, entries: same }
    );
  ...
  const invoiceFacts = {
    ...
    taxLines: taxLines.lines.map((l) => ({
      amount: l.amount,
      taxability_reason: l.taxability_reason ?? null
    })),
    hosted: typeof inv.hosted_invoice_url === 'string',
    pdf: typeof inv.invoice_pdf === 'string'
  };
  if (inv.status !== 'paid')
    throw new CampaignError(`the invoice is ${inv.status}, expected paid`, invoiceFacts);
  if (inv.total !== session.amount_total)
    throw new CampaignError(
      `the invoice total ${inv.total} is not the session total ${session.amount_total}`,
      invoiceFacts
    );
  if (inv.currency !== session.currency)
    throw new CampaignError(
      `the invoice is in ${inv.currency}, the session in ${session.currency}`,
      invoiceFacts
    );
  if (!invoiceFacts.hosted || !invoiceFacts.pdf)
    throw new CampaignError('the invoice has no hosted page or no PDF', invoiceFacts);
  const invoiceTax = taxLines.lines.reduce((s, l) => s + (l.amount ?? 0), 0);
  if (invoiceTax !== tax)
    throw new CampaignError(
      `the invoice's tax lines sum to ${invoiceTax}, the session's tax is ${tax}`,
      invoiceFacts
    );
  ...
  if (!mail)
    throw new CampaignError(`no mail "${subject}" reached ${spec.details.billingEmail} in 90 s`, {
      sessionId,
      subject
    });
  ...
  const mailFacts = {
    ...
    amountText,
    hasPdfLink: text.includes('Download the PDF: https://'),
    hasAmount: text.includes(amountText)
  };
  if (!mail.Subject.includes(spec.name))
    throw new CampaignError('the invoice mail subject does not name the organization', mailFacts);
  if (!mailFacts.hasPdfLink)
    throw new CampaignError('the invoice mail text carries no "Download the PDF: https://" line', mailFacts);
  if (!mailFacts.hasAmount)
    throw new CampaignError(`the invoice mail text does not carry the amount paid ${amountText}`, {
      ...mailFacts,
  ...
  const evidence = {
    ...
    sessionTaxBreakdown: (session.total_details?.breakdown?.taxes ?? []).map((t) => ({
      amount: t.amount,
  ...
  ctx.log(
    `buying: ${label} paid ${evidence.total} ${evidence.currency}, entry ${entry.id}, invoice ${invoiceId}`
  );
  return evidence;
}

/** Stripe's reason for a zero tax, from the invoice's tax line or else the session's breakdown. */
const taxabilityReason = (p) =>
  p.invoiceTaxLines[0]?.taxability_reason ?? p.sessionTaxBreakdown[0]?.taxability_reason ?? null;
...
export const scenarios = [
  {
    ...
    async run(ctx) {
      ...
      for (const [key, spec] of Object.entries(ORGS)) {
        ...
        if (!org)
          throw new CampaignError(`the hub answered no id for the new organization ${spec.name}`, { key });
        const afterDetails = await ctx.hub.patchAccount(org, spec.details);
        if (afterDetails.stripe?.customer !== true) {
          throw new CampaignError(`the PATCH of billing details made no Stripe customer (${key})`, {
            org,
            stripe: afterDetails.stripe
          });
        }
        let account = afterDetails;
        if (spec.currency !== 'EUR') {
          account = await ctx.hub.patchAccount(org, { currency: spec.currency });
          if (account.stripe?.customer !== true)
            throw new CampaignError(`the currency PATCH lost the Stripe customer (${key})`, {
              org,
              stripe: account.stripe
            });
        }
        if (account.currency !== spec.currency) {
          throw new CampaignError(`the ${key} account is in ${account.currency}, expected ${spec.currency}`, {
            org,
        ...
        if (account.currencyLocked !== false)
          throw new CampaignError(`the ${key} account is locked before any purchase`, {
            org,
            currencyLocked: account.currencyLocked
          });
        const customerId = await ctx.hub.customerOf(org);
        if (!customerId || !customerId.startsWith('cus_'))
          throw new CampaignError(`the ${key} account row carries no Stripe customer id`, {
            org,
            customerId
          });
      ...
      if (
        !settings ||
        typeof settings.customMinCredits !== 'number' ||
      ...
      return {
        ...evidence,
        customMinCredits: settings.customMinCredits,
  ...
  },
  {
    ...
    needs: NEEDS,
    run: (ctx) =>
      buy(ctx, 'be', { credits: 37000 }, { credits: 37000, subtotal: 3700, label: 'be-custom-37000' })
  },
  {
    ...
    async run(ctx) {
      ...
      const asked = floor - 1;
      const [before, ledgerBefore, sessionsBefore] = await Promise.all([
        ctx.hub.account(org),
        ctx.hub.ledger(org),
        sessionCount(ctx, customerId)
      ]);
      ...
      const answer = {
        ...
        sessionId: res.json?.sessionId ?? null
      };
      if (res.status !== 400 || code !== 'custom_amount_below_floor')
        throw new CampaignError(
          `a checkout for ${asked} credits (floor ${floor}) was not refused 400 custom_amount_below_floor`,
          answer
        );
      if (res.json?.url || res.json?.sessionId)
        throw new CampaignError('the refusal carries a session', answer);
      const [after, ledgerAfter, sessionsAfter] = await Promise.all([
        ctx.hub.account(org),
        ctx.hub.ledger(org),
        sessionCount(ctx, customerId)
      ]);
      if (after.balance !== before.balance || ledgerAfter.length !== ledgerBefore.length) {
        throw new CampaignError('the ledger changed on a refused checkout', {
          before: before.balance,
          after: after.balance,
          entriesBefore: ledgerBefore.length,
          entriesAfter: ledgerAfter.length
        });
      }
      if (sessionsAfter !== sessionsBefore)
        throw new CampaignError('Stripe lists a new Checkout session for the customer after the refusal', {
          sessionsBefore,
          sessionsAfter
        });
      return {
        ...
        floor,
        asked,
        accountFloor: before.customMinCredits ?? null,
        ...answer,
        balance: after.balance,
        entries: ledgerAfter.length,
        stripeSessions: sessionsAfter
      };
    }
  },
  {
    ...
    async run(ctx) {
      const { org, customerId } = orgOf(ctx, 'be');
      const [before, sessionsBefore] = await Promise.all([
        ctx.hub.account(org),
        sessionCount(ctx, customerId)
      ]);
      ...
      if (res.status !== 400 || error.code !== 'validation_error')
        throw new CampaignError('a checkout for pack 12345 was not refused 400 validation_error', answer);
      if (!JSON.stringify(error.details ?? error.message ?? '').includes('pack'))
        throw new CampaignError('the validation error does not name the field pack', answer);
      ...
      if (after.balance !== before.balance)
        throw new CampaignError('the balance changed on a refused checkout', {
          before: before.balance,
          after: after.balance
        });
      if (sessionsAfter !== sessionsBefore)
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (fromRate !== 1358)
        ctx.report.note(
          `buying: the USD rate in the settings is ${rate}, which prices 12,345 credits at ${fromRate}; the scenario still expects 1358.`
        );
      const evidence = await buy(
        ctx,
        'us',
        { credits: 12345 },
        { credits: 12345, subtotal: 1358, label: 'us-custom-12345' }
      );
  ...
  },
  {
    ...
    async run(ctx) {
      const evidence = await buy(
        ...
        'fr',
        { pack: 20000 },
        { credits: 20000, subtotal: 2000, label: 'fr-20000' }
      );
      if (evidence.tax !== 400 || evidence.total !== 2400)
        throw new CampaignError(
          `the French consumer paid tax ${evidence.tax} and total ${evidence.total}, expected 400 and 2400`,
          evidence
        );
      return evidence;
    }
  },
  {
    ...
    async run(ctx) {
      ...
      const evidence = await buy(
        ...
        'frb',
        { pack: 20000 },
        { credits: 20000, subtotal: 2000, label: 'frb-20000' }
      );
      if (evidence.tax !== 0 || evidence.total !== 2000)
        throw new CampaignError(
          `the French business paid tax ${evidence.tax} and total ${evidence.total}, expected 0 and 2000`,
          evidence
        );
      const reason = taxabilityReason(evidence);
      if (reason !== 'reverse_charge')
        throw new CampaignError(
          `the zero tax is not a reverse charge (taxability_reason ${reason})`,
          evidence
        );
      const taxIds = await ctx.stripe.get(`/v1/customers/${customerId}/tax_ids`);
      const ids = (taxIds.data ?? []).map((t) => ({
        type: t.type,
        value: t.value,
        verification: t.verification?.status ?? null
      }));
      if (!ids.some((t) => t.type === 'eu_vat' && t.value === 'FR32123456789'))
        throw new CampaignError('the Stripe customer does not list eu_vat FR32123456789', {
          customerId,
          taxIds: ids
        });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (!state(ctx).purchases.some((p) => p.key === 'be'))
        throw new CampaignError('no Belgian purchase was made before this scenario', {});
      const before = await ctx.hub.account(org);
      if (before.currencyLocked !== true)
        throw new CampaignError('the account is not locked after its first purchase', {
          currency: before.currency,
          currencyLocked: before.currencyLocked
        });
      if (before.currency !== 'EUR')
        throw new CampaignError(`the account is in ${before.currency}, expected EUR`, {
          currency: before.currency
        });
      ...
      else if (res.status === 200 && res.json?.currency === 'EUR')
        answered = '200 with the currency unchanged';
      else if (res.status === 200)
        throw new CampaignError(`the PATCH answered 200 with the currency ${res.json?.currency}`, {
          status: res.status,
          currency: res.json?.currency
        });
      else
        throw new CampaignError(
          `the PATCH answered ${res.status} ${code}, neither 409 currency_locked nor 200 unchanged`,
          { status: res.status, body: res.json ?? res.text?.slice(0, 400) }
        );
      const after = await ctx.hub.account(org);
      if (after.currency !== 'EUR' || after.currencyLocked !== true)
        throw new CampaignError('the currency changed after the refused PATCH', {
          currency: after.currency,
          currencyLocked: after.currencyLocked
        });
      return {
        org,
        answered,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (bought.length !== 4)
        throw new CampaignError(
          `${bought.length} Belgian purchases are on record, the four purchase scenarios must all be green first`,
          { purchases: bought.map((p) => p.sessionId) }
        );
      ...
      const rows = (invoices ?? []).map((i) => ({
        ...
        currency: i.currency,
        hosted: typeof i.hostedUrl === 'string',
        pdf: typeof i.pdfUrl === 'string'
      }));
      if (rows.length !== 4)
        throw new CampaignError(`the invoice list has ${rows.length} rows, expected 4`, { rows });
      const bad = rows.filter(
        (r) => r.status !== 'paid' || !r.hosted || !r.pdf || String(r.currency).toLowerCase() !== 'eur'
      );
      if (bad.length)
        throw new CampaignError('an invoice is not paid, not in EUR, or lacks its hosted page or PDF', {
          bad
        });
      const amounts = rows.map((r) => r.amountMinor).sort((a, b) => a - b);
      const expected = [2420, 4477, 6050, 12100];
      if (JSON.stringify(amounts) !== JSON.stringify(expected))
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const bought = state(ctx).purchases.filter((p) => p.key === 'be');
      if (bought.length !== 4)
        throw new CampaignError(`${bought.length} Belgian purchases are on record, expected 4`, {
          purchases: bought.map((p) => p.sessionId)
        });
      ...
      for (const p of bought) {
        ...
        const row = {
          ...
          entries: entries.map((e) => e.amount)
        };
        compared.push(row);
        if (entries.length !== 1)
          throw new CampaignError(`${entries.length} purchase entries for ${pi}, expected 1`, row);
        if (entries[0].amount !== Number(session.metadata?.credits))
          throw new CampaignError(
            `the entry is ${entries[0].amount} credits, the session metadata says ${session.metadata?.credits}`,
            row
          );
      }
      ...
      if (signup !== 5000)
        ctx.report.note(
          `buying: the Belgian organization's sign-up grant is ${signup}, not 5000; the balance is judged on the grant the ledger carries.`
        );
      const expected = signup + 20000 + 50000 + 100000 + 37000;
      const agrees = await ctx.hub.balanceAgrees(org);
      if (agrees.balance !== expected)
        throw new CampaignError(
          `the balance is ${agrees.balance}, expected ${expected} (sign-up ${signup} plus 207,000 bought)`,
          { agrees, compared }
        );
