...
// in `ctx.shared.refunds`, the evidence of the group in one place.
...
function need(value, what) {
  if (value === null || value === undefined) {
    throw new CampaignError(
      `${what} is missing: an earlier scenario of this group did not run or did not finish`,
      {}
    );
  }
  return value;
...
async function buy(ctx, org, body, { label, email }) {
  ...
  if (paid.outcome !== 'paid')
    throw new CampaignError('the Checkout page did not report a payment', {
      sessionId,
  ...
  const session = await waitFor(
    async () => {
      ...
      return s.payment_status === 'paid' && s.payment_intent && typeof s.payment_intent === 'object'
        ? s
        : null;
    },
  ...
  if (!session)
    throw new CampaignError('the Checkout session never read paid with a payment intent', { sessionId });
  const intent = session.payment_intent;
  const credits = body.pack ?? body.credits;
  const entry = await ctx.hub.waitLedger(org, { kind: 'purchase', sourceRef: intent.id });
  if (!entry)
    throw new CampaignError('no purchase entry for the payment intent within 90 s', {
      sessionId,
      paymentIntentId: intent.id,
      ledger: (await ctx.hub.ledger(org)).slice(0, 5)
    });
  if (entry.amount !== credits)
    throw new CampaignError('the purchase entry is not the credits bought', {
      paymentIntentId: intent.id,
      credits,
      entry
    });
  ...
  shared(ctx).paymentIntents.push({
    org,
    label,
    paymentIntentId: intent.id,
  ...
  return {
    sessionId,
    paymentIntentId: intent.id,
    chargedMinor: intent.amount_received,
    currency: intent.currency,
    entry,
    customerId
  };
...
function expectRefusal(answer, status, code, what) {
  if (answer.status !== status || answer.code !== code) {
    throw new CampaignError(
      `${what}: expected ${status} ${code}, answered ${answer.status} ${answer.code ?? ''}`,
      { answer: answer.body }
    );
  }
}
...
export const scenarios = [
  {
    ...
    async run(ctx) {
      ...
      if (!r.workspaceId || !r.org)
        throw new CampaignError('the Slideless workspace does not project a hub organization', { r });
      ...
      for (const [side, org, name] of [
        ...
        if (account.autoRecharge?.enabled)
          throw new CampaignError(
            `auto-recharge is on for the new organization ${name}: the arithmetic of this group assumes it off`,
            { org, autoRecharge: account.autoRecharge }
          );
        out[side] = {
          org,
          accountId,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (afterPurchase.balance !== before.balance + 20000) {
        throw new CampaignError('the balance after the purchase is not the previous balance plus 20,000', {
          before: before.balance,
          after: afterPurchase.balance
        });
      }
      // Spend down to 2,500 with debit events worth at most 5,000 each.
      const target = 2500;
      const toSpend = afterPurchase.balance - target;
      if (toSpend <= 0 || toSpend % 5 !== 0)
        throw new CampaignError(
          'the balance after the purchase cannot be spent down to 2,500 in multiples of 5',
          { balance: afterPurchase.balance }
        );
      const events = [];
      for (let left = toSpend; left > 0; left -= Math.min(5000, left))
        events.push(ctx.hub.debitEvent(R.org, Math.min(5000, left)));
      ...
      if (!debits)
        throw new CampaignError('not every debit event landed on the ledger', {
          events: events.length,
          landed: (await ctx.hub.ledger(R.org, 'debit')).filter((e) => ids.has(e.sourceRef)).length
        });
      const debitSum = debits.reduce((s, e) => s + e.amount, 0);
      if (debitSum !== -toSpend)
        throw new CampaignError('the debits do not sum to what was spent', { toSpend, debitSum });
      const agrees = await ctx.hub.balanceAgrees(R.org);
      if (!agrees.agrees || agrees.balance !== target)
        throw new CampaignError('the balance after spending is not 2,500 or does not agree with the ledger', {
          agrees,
          target
        });
      st.first = bought;
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (refund.status !== 'succeeded' && refund.status !== 'pending')
        throw new CampaignError('Stripe did not accept the full refund', {
          refundId: refund.id,
          status: refund.status
        });
      shared(ctx).refunds.push({
        id: refund.id,
        paymentIntentId: first.paymentIntentId,
        amountMinor: refund.amount,
        full: true
      });
      const entry = await ctx.hub.waitLedger(R.org, {
        kind: 'refund',
        amount: -20000,
        sourceRef: first.paymentIntentId
      });
      if (!entry)
        throw new CampaignError('no refund entry of -20,000 for the payment intent within 90 s', {
          refundId: refund.id,
          entries: await ctx.hub.entriesOf(R.org, first.paymentIntentId)
        });
      if (entry.remaining !== null)
        throw new CampaignError('the refund entry carries a remaining counter', { entry });
      const expected = spent - 20000;
      const agrees = await ctx.hub.balanceAgrees(R.org);
      if (!agrees.agrees || agrees.balance !== expected)
        throw new CampaignError('the balance after the refund is not the spent balance minus 20,000', {
          expected,
          agrees
        });
      if (agrees.balance >= 0)
        throw new CampaignError('the balance after the refund is not negative', { agrees });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (
        ...
        throw new CampaignError('the hub check did not refuse one MiB on the negative balance', {
          check,
          balance
        });
      }
      const upload = await ctx.sl.uploadAsset(R.workspaceId, 'x'.repeat(4096), {
        key: R.key,
        label: 'refund-negative'
      });
      const error = upload.json?.error ?? {};
      if (upload.status !== 402 || error.code !== 'entitlement_denied') {
        throw new CampaignError('Slideless did not refuse the upload with 402 entitlement_denied', {
          status: upload.status,
          body: upload.json ?? upload.text.slice(0, 400)
        });
      }
      const details = error.details ?? {};
      if (details.balance !== balance || typeof details.topUpUrl !== 'string' || details.topUpUrl === '') {
        throw new CampaignError(
          'the Slideless refusal does not carry the negative balance and a top-up link',
          { details, balance }
        );
      }
      const free = await ctx.hub.usageCheck(R.org, 'presentations.list', 1);
      if (free.allowed !== true || free.priced !== false)
        throw new CampaignError('an unpriced action is not allowed on the negative balance', { free });
      return {
        hubCheck: {
        ...
        slideless: {
          ...
          topUpUrl: details.topUpUrl
        },
        unpriced: {
          actionKey: 'presentations.list',
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const record = await ctx.relay.waitProcessed(
        (r) => r.type === 'charge.refunded' && r.body.includes(first.paymentIntentId),
        { timeoutMs: 30_000 }
      );
      if (!record) {
        throw new CampaignError('no processed charge.refunded for the payment intent at the relay', {
          paymentIntentId: first.paymentIntentId,
          seen: refundRecords(ctx, first.paymentIntentId).map((r) => ({
            id: r.id,
            outcome: r.outcome,
            status: r.status
          }))
      ...
      if (again.status !== 200 || again.answer?.outcome !== 'duplicate')
        throw new CampaignError('the second delivery was not answered duplicate', {
          eventId: record.id,
          again
        });
      const entries = await refundEntries(ctx, R.org, first.paymentIntentId);
      if (entries.length !== 1 || entries[0].amount !== -20000)
        throw new CampaignError('the payment intent does not carry exactly one refund entry of -20,000', {
          entries
        });
      const agrees = await ctx.hub.balanceAgrees(R.org);
      if (!agrees.agrees || agrees.balance !== st.balanceAfterRefund)
        throw new CampaignError('the balance moved on the second delivery', {
          expected: st.balanceAfterRefund,
          agrees
        });
      return {
        eventId: record.id,
        firstOutcome: 'processed',
        secondOutcome: again.answer.outcome,
        deliveries: record.deliveries,
        refundEntries: entries.length,
        balance: agrees.balance
      };
    }
  },
  {
    ...
    async run(ctx) {
      ...
      const bought = await buy(
        ...
        R.org,
        { credits },
        { label: `refund-custom-${credits}`, email: EMAILS.R }
      );
      ...
      const record1 = await ctx.relay.waitProcessed((r) =>
        refundRecords(ctx, bought.paymentIntentId, r1).includes(r)
      );
      if (!record1)
        throw new CampaignError('the first partial refund was not processed at the relay', {
          refundId: refund1.id,
          seen: refundRecords(ctx, bought.paymentIntentId).map((r) => ({ id: r.id, outcome: r.outcome }))
        });
      ...
      if (!one || one.length !== 1 || one[0].amount !== -expected1)
        throw new CampaignError('the first partial refund entry is not the floored proportion', {
          r1,
          amount,
          expected: -expected1,
          entries: one
      ...
      const record2 = await ctx.relay.waitProcessed((r) =>
        refundRecords(ctx, bought.paymentIntentId, r1 + r2).includes(r)
      );
      if (!record2)
        throw new CampaignError('the second partial refund was not processed at the relay', {
          refundId: refund2.id,
          seen: refundRecords(ctx, bought.paymentIntentId).map((r) => ({ id: r.id, outcome: r.outcome }))
        });
      ...
      if (!two || two.length !== 2 || totalTaken !== -expectedTotal) {
        throw new CampaignError('the two partial refunds do not total the floored cumulative proportion', {
          r1,
          r2,
          amount,
      ...
      if (again.status !== 200 || again.answer?.outcome !== 'duplicate')
        throw new CampaignError('the redelivered second refund was not answered duplicate', {
          eventId: record2.id,
          again
        });
      const after = await refundEntries(ctx, R.org, bought.paymentIntentId);
      if (after.length !== 2)
        throw new CampaignError('the redelivery wrote another refund entry', { entries: after });
      const agrees = await ctx.hub.balanceAgrees(R.org);
      const expectedBalance = before.balance + credits - expectedTotal;
      if (!agrees.agrees || agrees.balance !== expectedBalance)
        throw new CampaignError('the balance is not the purchase minus the refunded credits', {
          expectedBalance,
          agrees
        });
      ...
      return {
        ...
        refund2: {
          id: refund2.id,
          amountMinor: r2,
          entry: two.find((e) => e.id !== one[0].id)?.amount,
          eventId: record2.id
        },
        totalTaken,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (before.balance >= 0)
        throw new CampaignError(
          'the balance before the top-up is not negative: this scenario would prove nothing',
      ...
      if (!agrees.agrees || agrees.balance !== before.balance + 50000 || agrees.balance <= 0) {
        throw new CampaignError(
          'the balance after the top-up is not the previous one plus 50,000, above zero',
          { before: before.balance, agrees }
        );
      }
      const check = await ctx.hub.usageCheck(R.org, 'files.upload', MIB);
      if (check.allowed !== true || check.balance !== agrees.balance)
        throw new CampaignError('the check does not allow one MiB after the top-up', { check });
      return {
        paymentIntentId: bought.paymentIntentId,
        chargedMinor: bought.chargedMinor,
        balanceBefore: before.balance,
        balance: agrees.balance,
        check: { allowed: check.allowed, credits: check.credits, reason: check.reason }
      };
    }
  },
  {
    ...
    async run(ctx) {
      ...
      if (registered.status !== 200 || registered.body?.referral?.referredBy !== P.accountId) {
        throw new CampaignError("the referee could not register the referrer's code", {
          answer: registered.body,
          expected: P.accountId
        });
      }
      ...
      const row = await ctx.hub.sql(
        `SELECT id || '|' || state FROM referrals WHERE referee_account_id = '${E.accountId}'`
      );
      ...
      return {
        ...
        registered: {
          ...
          referredBy: registered.body.referral.referredBy,
          rewarded: registered.body.referral.rewarded
        },
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (!(refereeCredits > 0) || !(referrerCredits > 0)) {
        throw new CampaignError(
          'a referral reward is configured at 0 credits: the scenario cannot prove a reward of nothing',
          { refereeCredits, referrerCredits }
        );
      }
      const bought = await buy(ctx, E.org, { pack: 20000 }, { label: 'referee-pack-20000', email: EMAILS.E });
      const eEntry = await ctx.hub.waitLedger(E.org, {
        kind: 'referral',
        amount: refereeCredits,
        sourceRef: referralId
      });
      if (!eEntry)
        throw new CampaignError('no referral entry for the referee', {
          referralId,
          ledger: (await ctx.hub.ledger(E.org)).slice(0, 5)
        });
      const pEntry = await ctx.hub.waitLedger(P.org, {
        kind: 'referral',
        amount: referrerCredits,
        sourceRef: referralId
      });
      if (!pEntry)
        throw new CampaignError('no referral entry for the referrer', {
          referralId,
          ledger: (await ctx.hub.ledger(P.org)).slice(0, 5)
        });
      if (settings.promoGrantExpiryDays > 0 && (!eEntry.expiresAt || !pEntry.expiresAt)) {
        throw new CampaignError('a referral entry carries no expiry although promo grants expire', {
          promoGrantExpiryDays: settings.promoGrantExpiryDays,
          eEntry,
          pEntry
        });
      }
      const account = await ctx.hub.account(E.org);
      if (account.referral?.rewarded !== true)
        throw new CampaignError('the referee account does not read rewarded', { referral: account.referral });
      const state = await ctx.hub.sql(`SELECT state FROM referrals WHERE id = '${referralId}'`);
      if (state !== 'rewarded')
        throw new CampaignError('the referrals row is not rewarded', { referralId, state });
      ...
      const second = await buy(
        ...
        { label: `referee-custom-${customCredits}`, email: EMAILS.E }
      );
      ...
      if (eReferrals.length !== 1 || pReferrals.length !== 1) {
        throw new CampaignError('the second purchase changed the count of referral entries', {
          referee: eReferrals.length,
          referrer: pReferrals.length
        });
      }
      const eAgrees = await ctx.hub.balanceAgrees(E.org);
      const pAgrees = await ctx.hub.balanceAgrees(P.org);
      if (!eAgrees.agrees || !pAgrees.agrees)
        throw new CampaignError('a balance disagrees with its ledger', { eAgrees, pAgrees });
      return {
        referralId,
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (registered.status !== 200 || registered.body?.referral?.referredBy !== P.accountId) {
        throw new CampaignError(
          "a fresh organization could not register the referrer's code after the reward",
          { answer: registered.body }
        );
      }
      ...
      const pReferrals = await ctx.hub.ledger(P.org, 'referral');
      if (pReferrals.length !== 1)
        throw new CampaignError('a registration alone paid the referrer', { referrer: pReferrals.length });
      return {
        freshOrg: fresh,
        fresh: {
          status: registered.status,
        ...
        referee: `${again.status} ${again.code}`,
        referrerEntries: pReferrals.length
      };
    }
  },
  {
    ...
    async run(ctx) {
      ...
      await ctx.hub.patchAccount(late, {
        billingEmail: 'campaign-referral-late@drill.test',
        address: ADDRESS
      });
      const bought = await buy(
        ctx,
        late,
        { pack: 20000 },
        { label: 'referral-late-pack-20000', email: 'campaign-referral-late@drill.test' }
      );
      ...
      if (referrals.length !== 0)
        throw new CampaignError('the late organization holds a referral entry', { referrals });
      return {
        org: late,
        paymentIntentId: bought.paymentIntentId,
        refusal: `${refused.status} ${refused.code}`,
        referralEntries: referrals.length
      };
    }
  }
];
