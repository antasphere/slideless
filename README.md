...
| `packages/cli`      | Typed CLI over the SDK; the `platform` binary ([docs/cli.md](docs/cli.md)) |
...
DATABASE_URL=postgres://platform:<pw>@localhost:5432/platform pnpm --filter @platform/server dev
```

The `docs/` operator guide covers install, upgrade, backup, reverse proxy, and every env var; `docs/cli.md` covers the `platform` command-line client; `docs/instantiation.md` is the checklist for turning this template into a product (rename the `@platform/*` packages, image ref, scopes, and identity constants).
