#!/usr/bin/env bash
#
# ADVERSARIAL ARTIFACT (adv/fed-e2e) — NOT product source. A verbatim copy of
# scripts/federation-drill.sh with the ONE Phase-4 fix (add `-H "Origin: $SL"`)
# that lets the shipped drill reach a clean 14/14. The real fix belongs in
# scripts/federation-drill.sh via a normal fix lane; this copy is evidence only.
#
...
# ADVERSARIAL COPY: REPO points at the repo root (this file lives in adversarial/fed-e2e/)
