...
- **The CLI**: `slideless share <id> --name "Alice"` prints the URL; `--to-version`, `--annotator`,
  `--no-forms`, `--no-remember`, `--no-download`, `--no-bar`, `--expires`, `--password` set the rest, and `--embed`
...
unless they were switched off, the files unless downloads were switched off. What the recipient
