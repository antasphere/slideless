 ...
 *  - the CLI export command against a real listening server.
 *
...
beforeAll(async () => {
  ...
  const minted = await mintKey(ownerCookie, 'export-only', ['data:export']);
...
describe('GET /workspace/export', () => {
  ...
  it('machine access: data:export required, role still enforced, audited with the key identity', async () => {
    ...
    // the role gate.
    const memberKey = await mintKey(memberCookie, 'member-export', ['data:export']);
