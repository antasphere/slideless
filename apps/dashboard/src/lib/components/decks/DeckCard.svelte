<script lang="ts">
  /* One deck as a card (PRDCT-2437): a drawn plate, the title, and the two or
     three facts a person who is not technical cares about. No id, no hash, no
     version number. The plate is the deck's own: its pattern says what kind of
     deck it is, its field is seeded from the deck's id, so one deck keeps one
     face everywhere it shows. */
...
>
  <div class="plate-window plate" class:compact>
    <PatternCanvas {pattern} {palette} {seed} active={played} />
  ...
  <div class="body">
    ...
    <p class="facts">
      {#if deck.currentVersion > 0}
        ...
        <span class="fact">
          <Eye class="size-3.5" />
          {deck.totalViews === 1 ? t('decks.oneOpen') : t('decks.opens', { count: String(deck.totalViews) })}
        </span>
      {:else}
